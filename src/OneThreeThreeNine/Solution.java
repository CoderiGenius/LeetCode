...
public class Solution {
    ...
    public int maxProduct(TreeNode root) {
        ...
        helper(root,map);
        helper2(root,map);
        //System.out.println(max.longValue()%1000000007);
        Long tem = (max.longValue() % 1000000007);
        return  tem.intValue();
    }
    void helper(TreeNode root, Map<TreeNode,Integer> map){
        if(root==null)return;
        total += root.val;
    ...
    void helper2(TreeNode root,Map<TreeNode,Integer> map){
        if(root==null)return;
        //System.out.println(root.val+":"+map.get(root));
        BigDecimal bigDecimal = new BigDecimal((total-map.get(root)));
        BigDecimal bigDecimal1 = new BigDecimal(map.get(root));
         //long current = quickMultiply((total-map.get(root)),map.get(root));
        BigDecimal current = bigDecimal.multiply(bigDecimal1);
        System.out.println("bigDecimal:"+bigDecimal+" bigDecimal1:"+bigDecimal1+" current:"+current);
        //int current = Math.abs(total-map.get(root)-map.get(root));
        //System.out.println("root:"+map.get(root) + "total:"+(total-map.get(root)));

        //System.out.println(current);
        if(current.compareTo(max) > 0){
            max = current;
//            left = map.get(root);
//            right = total - map.get(root);
        }
        //max = Math.max(max,(total-map.get(root))*map.get(root));
       ...
       //String string = "2,3,9,10,7,8,6,5,4,11,1";
        ...
        while( i<strings.length){
            ...
            if(!queue.isEmpty() && !"null".equals(strings[i])) {
                ...
                right.val = Integer.parseInt(strings[i]);
    ...
    public static void helper2(TreeNode treeNode,String s){
        ...
        while (!queue.isEmpty()) {
            if (queue.peek() == null) {
                ...
                s2.append(queue.peek().val).append(",");
                queue.add(queue.peek().left);
                queue.add(queue.peek().right);
                queue.poll();
            }
        }
        System.out.println("same:"+s2.substring(0,s2.toString().length()-1).equals(s));
        System.out.println("same2:"+s2.substring(0,s2.toString().length()-1));
        System.out.println("sames:"+s);
        System.out.println(s2.substring(0,s2.toString().length()-1).replace(s,""));
