...
//        BigDecimal bigDecimal1 = new BigDecimal(map.get(root));
        //long current = (long) (total - map.get(root)) *map.get(root);
        int left = map.get(root);
        int right = total - map.get(root);
        ...
        if(Math.abs(right-left)<max){
            max = Math.abs(right-left);
            this.left = map.get(root);
            this.right = total - map.get(root);
        }
